...
Usage:
    python main.py --categorize
        Scan all emails, apply labels, auto-clean
    python main.py --categorize --dry-run
        Preview without changes
    python main.py --triage
        Score unread emails 1-10, show top 20
    python main.py --review
        List emails flagged for manual review
    python main.py --receipts
        Find and label receipt emails
    python main.py --max 5000
        Process up to N emails (default: all)
"""
...
from organizer.triage import list_review_me, triage_inbox


def main() -> None:
    """Parse CLI arguments and dispatch to the appropriate organizer module.
    ...
    mode.add_argument(
        ...
        action="store_true",
        help=(
            "Scan all emails, apply 5-category labels, "
            "auto-archive/delete stale bulk mail"
        ),
    )
    mode.add_argument(
        "--triage",
        action="store_true",
        help=(
            "Score every unread email 1-10 by priority, "
            "print top 20 to action today"
        ),
    )
    mode.add_argument(
        "--review",
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Max emails to process for --categorize "
            "(default: 0 = no limit)"
        ),
    )
    parser.add_argument(
